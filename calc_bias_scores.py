...
# python calc_bias_scores.py --embed_folder data/embed/wikitext-103/sgns --vocab_path data/embed/wikitext-103/vocab.txt --embed_type sgns --bias_score_path data/results/bias_scores/wikitext-103/sgns.pkl
...
def calc_bias_scores(target_words, gbps, embed_model):
    ...
    bias_scores = [bias_measure.batch_db_wa_bias(target_words, gbps, embed_model).tolist(),
                   bias_measure.batch_ripa_bias(
                       target_words, gbps, embed_model).tolist(),
...
def main():
    ...
    gbps = [tuple(gbp) for gbp in dl.load_gbp()
            if gbp[0] in vocab and gbp[1] in vocab]
    ...
    if embed_type == 'sgns':
        ...
        for embed_idx, embed_path in enumerate(embed_paths):
            ...
            logger.info(
                f'Processing embedding model #{embed_idx+1} out of {len(embed_paths)}')
            embed_model = dl.load_gensim_sgns(embed_path)
            bias_scores.append(calc_bias_scores(
                list(vocab), gbps, embed_model))
    ...
    # save
    print(bias_scores)
    # measure, target, gbp, embed
    bias_scores = np.moveaxis(np.array(bias_scores), 0, 3)
    utils.write_pickle(
        [[bias_measures, list(vocab), gbps, model_names], bias_scores], bias_score_path)
